...
def find_peak(matrix,n,m):
    ...
    while(start <= end):
        mid = (start+end)//2
        maxInRow = max(matrix[mid])
        maxColmIndex = matrix[mid].index(maxInRow)
        if((mid == 0 or matrix[mid][maxColmIndex] > matrix[mid-1][maxColmIndex])and(mid == n-1 or matrix[mid][maxColmIndex] > matrix[mid+1][maxColmIndex])):
            peak = [mid, maxColmIndex]
            return peak
        elif matrix[mid][maxColmIndex] < matrix[mid-1][maxColmIndex]:
            end = mid-1
        else:
            start = mid+1
...
#USING ITERATIION, row column altered here
